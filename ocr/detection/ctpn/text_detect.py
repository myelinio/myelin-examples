...
def load_tf_model(checkpoints_path):
    # load config file
    cfg.TEST.checkpoints_path = checkpoints_path
    ...
    # load model
    print('Loading network {:s} from {:s}... '.format("VGGnet_test", cfg.TEST.checkpoints_path))
...
def text_detect(img):
    sess, net = load_tf_model('./ctpn/checkpoints')
    scores, boxes, img, scale = ctpn(sess, net, img)
