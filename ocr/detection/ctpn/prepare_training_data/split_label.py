...
path = '/media/ryadh/DATA4T/Ryadh_data/data/ctpn/VOC2007/JPEGImages'
gt_path = '/media/ryadh/DATA4T/Ryadh_data/data/ctpn/VOC2007/Annotations'
out_path = 're_image'
...
def get_boxes(gt_file):
    # global img_size, re_size, f, lines
...
for file in files:
    ...
    # gt_file_xml = os.path.join(gt_path, 'gt_' + stem + '.xml')
    gt_file = os.path.join(gt_path, 'gt_' + stem + '.txt')
    lines, img_size, re_size = get_boxes(gt_file)
    for line in lines:
