...
def general_crop_expand(image, tile):
    ...
    expand_ratio = 0.01
    expand_dist_w = int(bb_width * expand_ratio)
    expand_dist_h = int(bb_height * expand_ratio)
    x_min = max(newCX - int(round(bb_width / 2)) - expand_dist_w, 0)
    x_max = min(newCX + int(round(bb_width / 2)) + expand_dist_w, nW)
    y_min = max(newCY - int(round(bb_height / 2)) - expand_dist_h, 0)
    y_max = min(newCY + int(round(bb_height / 2)) + expand_dist_h, nH)
    print([x_min, y_min, x_max, y_max])
    print([nW, nH])
    return im_rotate[y_min:y_max, x_min:x_max, :], horiz
...
def crop_image(image_path, detection_results, crop_dir):
    ...
    for result in detection_results:
        ...
        save_crop_path = os.path.join(crop_dir, crop_name)
        ...
        crop_files.append(save_crop_path)
        crops.append(crop)

    return crop_files, crops

def crop_image_in_memory(img, detection_results):
    crops = []
    for result in detection_results:
        score = result[-1]
        x1 = result[0]
